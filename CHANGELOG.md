...
- **The emailed confirmation link leads somewhere.** `GET /api/subscribers/confirm-email?token=` has been the URL in every confirmation email since the dispatch app was written, with no function behind it. It exists now, and redirects to `/subscribe/confirmed?status=` on the rider app rather than answering JSON at someone who just clicked a link in their mail client. The token is never reflected back into the redirect - it would land in browser history and in any referrer. This is a GET that changes state, which is right in exactly this one place: the rider's click *is* the request, and the token is single-use and expiring. The cost is named rather than ignored - a mail scanner that prefetches links will confirm on the rider's behalf, which is precisely why the SMS channel uses a code someone has to type.
- **A texted code can be typed into the page.** `POST /api/subscribers/confirm-sms` takes the number and the six digits, normalizing the number server-side through the same rule the opt-in form uses (1.5.188) rather than trusting the browser to have done it. A wrong code and a number with no pending confirmation are **one answer**: this endpoint accepts a phone number from anyone, and distinguishing them would let a caller ask whether any given number is partway through signing up for MVTA alerts. Expiry, supersession and lockout keep their own names, deliberately - each reveals only that the number has an unconfirmed confirmation, and a rider whose code expired can do nothing with "that didn't work" and everything with "that expired, here is a new one".
- **A rider holding two texts is told to use the newer one.** After a resend the older code still looks current, and treating it as a wrong guess both misdescribed it and spent one of the five attempts. A code that was genuinely sent to that number is now reported as superseded (or as already confirmed) without counting against the cap; a code that was never sent to it still counts. This gives a guesser nothing - reaching it means naming a code actually issued to that number, which is exactly as hard as naming the live one. Found by a contract test written with the wrong expectation, which turned out to be describing better behaviour than the code had.
- **`POST /api/subscribers/resend` for a rider who never got theirs.** One token per channel at a time: the new one supersedes the old, because two live codes means a rider holding two texts, either of which looks current. Not more than one every two minutes, because a resend endpoint with no floor is a way to make OnBoard text a number as fast as anyone can post to it. A resend also clears a lockout - the attempt count belongs to the confirmation, not the subscriber, so a rider who mistyped five times is not locked out forever. It answers 202 identically whether a token was issued, whether the rider asked again too soon, and whether the contact exists at all; anything else would be the same enumeration oracle by a different door. Nothing is sent to a confirmed channel or to someone who opted out, since re-texting a person who asked us to stop - because they typed their number into a form - is the failure that guard exists for.
- **Token issuing is one implementation.** `lib/confirmationTokens.ts` holds the draw, the collision redraw and the insert; opt-in and resend both call it rather than keeping two copies of a retry loop only one of them was ever exercised with.
- **`normalizeUsPhone` now exists server-side too,** as a deliberate second copy - `@mvta/shared` is a frontend workspace package the Function App does not depend on. Both copies run the same table of inputs in their own tests, so the pair drifting apart fails a test rather than producing a number the form posts and the endpoint cannot match.
