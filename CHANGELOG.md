...
- **Behaviour to know.** The monthly summary now counts cases that are Ready for review or Reviewed (it counted `source_verified` rows, which included unreviewed Spare and cancellation cases and left out unconfirmed GTFS cases). `routes_affected_count` counts routes with a case flagged missed. The migration-121 column probes are gone from these paths. The four review outcomes, superseding reviews, the Expected-operating-window hold and the rebuilt `vw_MissedTrip` come with migration 124 (123 was taken).
