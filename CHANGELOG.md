...
- **Migration 126** adds `zone_version_sha256` with a filtered unique index (`UX_OnDemandOperationalZoneVersions_Identity`), plus `last_seen_at`, `last_seen_feed_version` and `unmonitored_locations_json`. Re-runnable. Versions imported before it have no identity and are never matched; dev has none. **Apply it before the first 09:30 UTC pull after this deploys**: the merge sets the feed URL, and until the columns exist the pull refuses and records "apply migration 126" as its failure reason rather than a SQL error.
...
- **Verified.**
  ...
  - `onDemandZoneImport.test.ts`: a pull on a database without migration 126 fails with "apply migration 126".
