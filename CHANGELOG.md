...
- **The number is normalized server-side, not trusted from the form.** `lib/phone.ts` is `normalizeUsPhone` again (1.5.188 added it to the rider app), because the Function App does not build the frontend workspace; both copies assert the same cases so they cannot drift quietly. At the form it is a convenience. Here it is correctness: numbers are stored in E.164, and "952-388-3275" arriving unchanged matches nothing, which reads to the rider as never having subscribed.
