...
- **Three ways to look like a missed trip that are not one.** The fixed-route silent-no-show detector infers a missed trip from absence: a scheduled trip past its 30-minute deadline with no positive vehicle-start evidence. It already refused to read absence as a no-show while `gtfs_vehicle_positions` was not current (migration 087). Three other things produce the same silence, all of them for a whole day at a time, so each arrives as a queue of hundreds of findings no reviewer can disprove. Items 2, 3 and 4 of the false-positive analysis in `plans/missed-trip-detection-logic-gaps.md`.
