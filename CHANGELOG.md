...
- **The Dispatch Log's Block is the workbook's Vehicle Block Id.** `shapeTrip` converts the GTFS `block_id` the log stores (`1-v64`, `24-v64`, `501-v64`) into the number the OCS workbook prints (`001`, `024`, `501`): the `-vNN` service-change suffix belongs to the feed, not to the block, and the workbook pads to three digits. Confirmed against the scheduling export and the live feed - route 495's 03:20 out of Mystic Lake is `1-v64` in GTFS and `001` in the export, `501-v64` and `501` on the weekend sheet. Converted on read, so days already materialized read right and no migration is needed; the stored value stays as the feed wrote it, because missed-trip block grouping needs the version to tell two service changes apart. The grid, watch queue, inspector, timeline lanes, search and the CSV `Block` column all follow. `CONTEXT.md` defines **Block**.
- **The Verified cell says when it was initialled.** The new `VerifiedInitials` puts the entry's time under the initials in both the grid and the watch queue (`verifiedAtLabel`, pinned to `AGENCY_TIME_ZONE` = `America/Chicago`, the same clock the service-date rule uses, so a browser outside Central cannot print a time that disagrees with the day rule applied to it; the inspector's Verification line now reads the same way). It is the *current* entry's time, so a correction re-times the cell, with the earlier entry kept in the history below. An entry recorded on a different calendar day from the service date - a past-midnight trip initialled at 01:12, or a next-morning correction - names its day ("Sep 9, 1:12 AM") so it cannot be misread. The full account name stays on hover. The CSV's existing `Verified At (UTC)` column is unchanged.
- **The audit trail is readable.** `GET /api/trip-start-log/history?date=&trip_id=` returns every change to one trip's cell from `TripStartVerificationEvents` (migration 096a, written since 1.5.107 and never read back), newest first: who, initials, from what, to what, note, when. Same read roles as the log itself - the record of what the SST OCS desk did is what MVTA staff reviewing the log need - and a database without the table answers an empty list, not an error. The inspector shows it as **History**, loaded per selected trip rather than per day (~600 rows), re-read after an entry is recorded, and absent entirely for a trip nobody has touched. Clearing appears as "Observed on time → cleared"; the grid still shows only where the cell stands now.
- **An event carries its row id.** `TripStartVerificationEvents.id` is returned with each event. The table ties on `recorded_at` at the second, and the ordering already tie-breaks on `id`; the console lists rows by it, so a cycle click through blank → on time → left late does not lose an entry to a duplicate key.
- **Tests.** `shapeTrip` block conversion, `tripStartLogHistory` event shaping, the handler's role and argument refusals, `verifiedAtLabel` against the service date, and console tests for the timed cell, the named day, the history list, its reload after recording, and its absence. No migration.
## [1.5.245] - 2026-09-17
