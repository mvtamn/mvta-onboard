...
- **No personal names.** `operator_name` and `driver_name` are omitted in favour of `logon_id` and `driver_identifier`. The standard measures the contractor, not an individual operator, and a dataset shared beyond the console is the wrong place for that difference to stop mattering.
