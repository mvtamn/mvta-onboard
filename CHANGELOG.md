...
- **A stopped channel is shown but cannot be switched back on here.** The API would return it to "waiting for confirmation" without sending a new link or code, and this page cannot send one because it only ever sees the contact masked - so it would leave the rider waiting on a confirmation that never arrives. Signing up again is the path that works. That gap in the API's revive path is noted for a follow-up rather than widened into this change.
...
- **Turning a channel back on does not re-prove it.** A channel that was confirmed stays confirmed, but one that was stopped returns to `pending_confirmation` rather than being promoted by a re-ticked box - re-ticking is not proof of a contact, and dispatch will not send to it until it is confirmed again. Both channels off opts the record out, the same rule `optOut` applies: a subscriber with nothing to send on is not a subscriber. An opted-out record is refused outright, because coming back is a new consent and the date someone agreed is what a TCPA complaint turns on.
