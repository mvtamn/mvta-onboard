...
- **Operational zones can be imported for the first time.** `loadOperationalZonesFromGtfsFlexArchive` has parsed GTFS-Flex archives since #92, but nothing ever called it outside its own test, so `OnDemandOperationalZoneVersions` has never held a row and `loadActiveOperationalZones` has always returned an empty set — the on-demand monitor has never run against real geometry, and could not be made to by configuration. A new import core (`lib/onDemandZoneImport.ts`) takes archive bytes, validates them through the existing parser before touching the database, hashes the source, and writes the version and its zones in one transaction; identical bytes re-imported under the same feed version are a no-op rather than a competing version, since `UQ_OnDemandOperationalZoneVersions_Source` is `(feed_version, source_sha256)`. Three endpoints drive it: `GET /manage/on-demand-zones` lists versions and names the active one for any staff reader, `POST /manage/on-demand-zones/import` takes the archive as the request body, and `POST /manage/on-demand-zones/{versionId}/activate` swaps the active version. Import and activation are separate because migration 074 says so — `is_active` defaults to 0, `imported_by` is recorded, and a filtered unique index permits exactly one active version — so an import never changes what live monitoring classifies against until someone decides it should. Activation deactivates and activates inside one transaction (the filtered index rejects the naive order), refuses a version with no zones, and invalidates the serving worker's zone cache. No new Entra role: `OCC.Admin` imports and activates, staff readers list. No migration; no schema change. Routes are under `manage/`, not the runtime-reserved `admin/`. Runbook: `docs/runbooks/on-demand-operational-zones.md`. Activating a version does not turn the monitor on — that still needs `ON_DEMAND_MONITORING_ENABLED` and a successful hourly reconciliation.
