...
- **Garage Departures, On-Demand: the same reading as Fixed Route.** The On-Demand view had the shape the Fixed Route view was just given (a Source column, an Outcome, the allowance in the card label) but none of the reading: the stored service date printed as `20260905`, Scheduled and Actual repeated the date in the browser's zone, the duty's requested start was an asterisk with a footnote, and the driver, vehicle and fallback duty id were 36-character Spare ids in full under headers that promised a driver and a vehicle. `GET /on-demand-departures` now judges each row with `lib/onDemandDepartureOutcome.ts` and returns an `outcome` (`late`, `no_departure`, `departed`, `pending`, `cancelled`, `no_schedule`) plus `judged_count` and `today_service_date` in diagnostics; the late and no-departure counts and the average are taken over judged duties only, so a cancelled or not-yet-due duty no longer dilutes them. The console groups duties under a band per service day, driver or vehicle (by Spare reference, with a Repeat flag), formats the day and Central times the way the fixed-route view does with the scheduled and actual source printed under each time instead of the asterisk, shows driver, vehicle and unidentified duties as an eight-character reference with the full id on hover, says "No driver on duty" instead of a dash, adds a Flagged-only filter and a per-day strip, and notes in the toolbar that on-demand is not yet a scored standard (ADR 0028). Late stays amber here, since no compliance candidate is raised from these duties yet; no departure is red in both views. The per-day strip and the Group by control moved into the shared module code both views use. No migration.
