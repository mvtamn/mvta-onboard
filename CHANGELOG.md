...
- **Every alert email carries a way to change or stop it.** Increment D of `plans/rider-preference-management-spec.md`. The footer says why the email arrived and links to the preference page from 1.5.200 - which until now nothing linked to. The footer is built in `functions-dispatch/src/lib/alertEmail.ts`, and the link is never logged, because it carries the manage key.
...
- **A rider can change their own subscription.** `GET` / `PUT /api/subscribers/preferences` and `POST /api/subscribers/unsubscribe`, authenticated by the manage key from 1.5.197. Increment B of `plans/rider-preference-management-spec.md`; the page that calls it is increment C.
