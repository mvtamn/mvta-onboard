...
- **`.stat-card` had no rule anywhere in the console.** `.stat-grid` was styled; the card inside it and its three children were not, so every card in every stat grid rendered as three unstyled stacked `div`s on the page background — the OTP Dashboard's four figures, Subscribers' pair, and the threshold preview on the OTP admin page. Both OTP and Subscribers passed a per-card `borderLeftColor` inline, which had no border to colour and had been inert since it was written. The rule added here keeps that left border, so the severity colours those two callers already pass finally appear, and the fix reaches all three pages at once.
- **The OTP trend chart drew its own threshold.** The "Routes below target" card read `diagnostics.target` from the feed while the chart four lines below decided `meets`/`below` against a literal `85`. Nothing enforced that they matched: a contract target that moved off 85% would have coloured bars against the old figure while the card counted routes against the new one, and neither would have looked wrong. The chart takes the target as a prop now, rounded once at the source because the wire value is a fraction and `0.85 * 100` is `85.00000000000001` — which both the card's label and the target line render as text.
- **The chart answers the compliance question rather than the height question.** The target is drawn as a dashed line with the passing range shaded above it, and the plot is 200px rather than 120px: six months of agency OTP span a handful of points, which on the old track was about ten pixels of difference between "breaching" and "compliant". The 0–100 scale is unchanged — bar height is still the month's own `pct_ontime`, so nothing is exaggerated by a clipped axis. Columns flex instead of carrying a fixed width, so the bars fill whatever width the panel gives them.
- **`--late-orange` is a light-mode token that was never given a dark counterpart.** Declared once on `.otp-panel` and left there, it sat at roughly 2.4:1 on the dark surface, so the below-target bars and the late segment of the adherence strip went muddy under the theme toggle. Lightened under `[data-theme="dark"]` the way `--brand-green-text` and `--success-text` already are.
- **The module header replaces the repeated banner.** Every OTP page stacked a `.panel-desc` line and a full-width accent-blue `.concept-banner` carrying the feed's status and the month picker. The page's name and purpose are a heading now, with the live/preview state as a status pill beside the control that changes it, and the feed's provenance as a caption underneath — the same anatomy Integrations & Data Health uses, which is the console's newest header vocabulary.
- **Dashboard tiles carry severity semantically.** Tone follows the number: nothing left to review is a settled state, not a warning-coloured zero. The tiles also split the figure from its unit and drop the accent left border in favour of a tone dot, matching `.ih-stat`.
