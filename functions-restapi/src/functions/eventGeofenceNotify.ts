...
app.serviceBusQueue("eventGeofenceNotify", { connection: "ServiceBusConnection", queueName: "event-geofence-notifications", handler: async (message: unknown, context: InvocationContext) => {
  ...
  const row = (await pool.request().input("id", sql.BigInt, id).query(`SELECT c.*,g.name geofence_name,matched.send_mode FROM EventGeofenceCrossings c JOIN EventGeofences g ON g.id=c.geofence_id OUTER APPLY (SELECT TOP 1 rule.send_mode FROM EventGeofenceDirectionRules rule WHERE rule.geofence_id=c.geofence_id AND rule.transition=c.transition AND (rule.heading_min <= rule.heading_max AND c.heading_at_crossing BETWEEN rule.heading_min AND rule.heading_max OR rule.heading_min > rule.heading_max AND (c.heading_at_crossing >= rule.heading_min OR c.heading_at_crossing <= rule.heading_max))) matched WHERE c.id=@id`)).recordset[0] as { vehicle_id: number; transition: string; geofence_name: string; destination_label: string | null; send_mode: "manual" | "auto" } | undefined;
