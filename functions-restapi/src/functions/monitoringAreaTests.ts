...
async function listTests() {
  ...
  const tests = (await pool.request().query(`
    ...
    FROM EventMonitoringAreaTests t
    JOIN EventLocations l ON l.id=t.location_id JOIN EventGeofences g ON g.id=t.geofence_id
    OUTER APPLY (SELECT TOP 1 status,exited_at FROM EventMonitoringAreaTestMessages m WHERE m.test_id=t.id ORDER BY m.created_at DESC) latest
...
app.http("monitoringAreaTests", { route: "monitoring-area-tests", methods: ["GET", "POST"], authLevel: "anonymous", handler: async (req: HttpRequest) => {
  ...
  const test = (await request.query<{ id: string }>(`
    MERGE EventMonitoringAreaTests WITH (HOLDLOCK) AS target
    ...
    OUTPUT INSERTED.id;
  `)).recordset[0];
  await pool.request().input("test", sql.UniqueIdentifier, test.id).query("DELETE FROM EventMonitoringAreaTestVehicleState WHERE test_id=@test");
...
app.http("monitoringAreaTestDisable", { route: "monitoring-area-tests/{id}", methods: ["DELETE"], authLevel: "anonymous", handler: async (req: HttpRequest) => {
  ...
  return result.rowsAffected[0] ? { status: 204 } : { status: 404, jsonBody: { error: "Active Monitoring Area test not found" } };
} });
