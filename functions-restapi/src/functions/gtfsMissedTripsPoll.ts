...
import { gtfsObservations } from "../lib/missedTripCase/adapters/gtfs";
...
app.timer("gtfsMissedTripsPoll", {
  ...
  handler: async (_timer: Timer, context: InvocationContext) => {
    ...
    const { observations, tally } = await gtfsObservations(feed.Entities, context, settings);
    ...
    context.log(
      `Missed-trip poll: ${feed.Entities.length} entities seen; observed ${tally.cancellations} cancellations, ` +
        ...
        `(silent no-show enabled=${settings.silentNoShowEnabled}). Cases: ${report.created} created, ${report.held} held, ` +
