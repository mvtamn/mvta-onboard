...
import { runFeedIngestion, type FeedRunReport } from "../lib/feedRun";
...
app.timer("gtfsStopsSync", {
  ...
  handler: async (_timer: Timer, context: InvocationContext) => {
    ...
    await runFeedIngestion("gtfs_static", context, async () => {
      const { stops, trips, routes, scheduledTrips, calendar, calendarDates, stopRoutes } =
        await fetchAndParseStatic(feedUrl);
      // An archive that parses to nothing is a broken delivery, not an empty
      // schedule: the existing tables are kept, and the ledger says why rather
      // than going on reading as the last good import.
      if (stops.length === 0 && trips.length === 0 && routes.length === 0) {
        return {
          kind: "failed",
          reason: "Static GTFS feed parsed to zero stops/trips/routes - keeping the existing data.",
        };
      }
      ...
      try {
        ...
        if (coverage && !coverage.covered_today) {
          ...
          const imported = stops.length + trips.length + routes.length;
          report = { kind: "stored", received: imported, stored: imported, coverage: { startAt: new Date(), endAt: new Date() } };
        ...
        // no-show detector will keep reading as today's expected service, so the
        // failure has to reach the ledger.
        throw err;
      }
    });
  },
