...
app.timer("spareMissedTripsIngest", {
  ...
  handler: async (_timer: Timer, context: InvocationContext) => {
    ...
    // Everything below runs inside runFeedsIngestion so a throw cannot leave the
    ...
    await runFeedsIngestion(["spare_requests", "spare_slots"], context, async () => {
      ...
      // received is what was written, not what was fetched: the upserts decline
      // out-of-scope services and unusable rows on purpose, and a write that
      // fails throws out of this run rather than being counted as a shortfall.
      return {
        spare_requests: { kind: "stored", received: requestWrites, stored: requestWrites, sourceTimestampSeconds: maxRequestUpdatedAt || null },
        spare_slots: { kind: "stored", received: slotWrites, stored: slotWrites, sourceTimestampSeconds: maxSlotUpdatedAt || null },
      };
    });
  },
});
