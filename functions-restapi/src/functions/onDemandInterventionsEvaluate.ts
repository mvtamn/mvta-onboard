...
app.timer("onDemandInterventionsEvaluate", {
  ...
  handler: async (_timer: Timer, context: InvocationContext) => {
    ...
    const { state, lastAuthoritativeReconciliationAt } =
      onDemandMonitoringStatus(true, await loadKpiFeedHealthRecords(pool));

    // Degraded means the On-Demand KPI trust stream is not Current: the last
    // authoritative reconciliation is too old to support a claim about now.
    // Records may still be arriving by webhook, but acting on them here would
    // create rider-facing drafts from a source the console is simultaneously
    // telling operators not to trust. Read-only until reconciliation recovers,
    // which is the same rule the workspace applies to its own actions - and,
    // because both read one ledger, the same rule Suggested Alert preparation
    // applies in ensureSuggestedAlert.
    if (state !== "current") {
      context.log(`On-demand intervention evaluation stood down: monitoring is ${state}.`);
      return;
    }
    if (!lastAuthoritativeReconciliationAt) return;

    await evaluateOnDemandInterventions(pool, new Date(), lastAuthoritativeReconciliationAt);
  },
});
