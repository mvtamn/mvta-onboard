...
import { rampUpStage } from "../lib/assessment/rampUp";
...
app.http("assessmentPeriodsOpen", {
  ...
  handler: async (request: HttpRequest, context: InvocationContext) => {
    ...
    try {
      ...
      const contractor = await req.query<{ contract_start_date: string }>(`SELECT contract_start_date FROM Contractors WHERE id=@contractor AND is_active=1`);
      if (!contractor.recordset[0]) return { status: 404, jsonBody: { error: "Active contractor not found" } };
      const stage = rampUpStage(contractor.recordset[0].contract_start_date, body.service_month);
      const write = pool.request(); write.input("contractor", sql.UniqueIdentifier, body.contractor_id); write.input("month", sql.Char(6), body.service_month); write.input("stage", sql.NVarChar(20), stage);
      const result = await write.query<{ id: string }>(`
        IF NOT EXISTS(SELECT 1 FROM AssessmentPeriods WHERE contractor_id=@contractor AND service_month=@month)
          INSERT AssessmentPeriods(contractor_id,service_month,ramp_up_stage) VALUES(@contractor,@month,@stage);
  ...
  handler: async (request: HttpRequest, context: InvocationContext) => {
    ...
    try {
      ...
      const result = await req.query<{ changed: number }>(`
        ...
        WHERE id=@id AND status='in_review' AND computed_revision=input_revision
          AND NOT EXISTS(SELECT 1 FROM PeriodKpiAssessments WHERE period_id=@id AND (manager_action='pending' OR reviewed_input_sha256<>input_sha256 OR ISNULL(data_completeness_pct,0)<=0));
        SELECT @@ROWCOUNT changed;
      `);
