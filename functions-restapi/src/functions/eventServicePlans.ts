...
app.http("eventServicePlans", { route: "event-service-plans", methods: ["GET", "POST"], authLevel: "anonymous", handler: async (req: HttpRequest) => {
  ...
  if (req.method === "GET") {
    ...
    const links = (await pool.request().query("SELECT 'routes' kind,service_plan_id,CONVERT(nvarchar(36),route_id) value FROM EventServicePlanRoutes UNION ALL SELECT 'geofences',service_plan_id,CONVERT(nvarchar(36),geofence_id) FROM EventServicePlanGeofences UNION ALL SELECT 'locations',service_plan_id,CONVERT(nvarchar(36),location_id) FROM EventServicePlanLocations")).recordset;
    return { status: 200, jsonBody: { plans: plans.map((plan) => ({ ...plan, links: links.filter((link) => link.service_plan_id === plan.id) })) } };
  }
