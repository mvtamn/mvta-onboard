    ...
    try {
      ...
      for (const contractor of contractors.recordset) {
        try {
          ...
          const plan = monthBoundaryPlan({ now, priorStatus: await statusOf(priorMonth), currentStatus: current });
          for (const month of [plan.openPrior, plan.openCurrent]) {
            if (!month) continue;
            const covered = pool.request(); covered.input("contractor", sql.UniqueIdentifier, contractor.id); covered.input("month", sql.Char(6), month); covered.input("agreement", sql.UniqueIdentifier, contractor.agreement_id);
            const window = (await covered.query<{ assigned: number; covers: number }>(`SELECT ${assignedStandardCountSql(scope)} assigned,CASE WHEN CONCAT(@month,'01') BETWEEN CONVERT(char(8),a.starts_on,112) AND CONVERT(char(8),a.ends_on,112) THEN 1 ELSE 0 END covers FROM Contractors c JOIN PerformanceAgreements a ON a.id=@agreement WHERE c.id=@contractor`)).recordset[0];
            if (!window?.covers) { context.log(`${contractor.name}: Agreement does not cover ${month}; not opened.`); continue; }
            if (!window.assigned) { context.warn(`${contractor.name}: no standards assigned for ${month}; not opened.`); continue; }
