...
// Staff upload under evidence/<assessment>/ with a create+write SAS. Once the
// bytes are verified the app copies them to sealed/<assessment>/<evidence id>
// with its own credential and registers that path; the SAS never names a
// sealed/ path, so the holder cannot alter what the hash was taken from
// (ADR 0013). The staging blob is deleted afterwards - it is not an artifact.
export function isEvidenceStagingPath(blobPath:string,assessmentId:string):boolean{return blobPath.startsWith(`evidence/${assessmentId}/`)&&!blobPath.includes("..");}
export function sealedComplianceEvidencePath(assessmentId:string,evidenceId:string,fileName:string):string{const ext=(fileName.match(/\.[a-zA-Z0-9]{1,8}$/)?.[0]??"").toLowerCase();return `sealed/${assessmentId}/${evidenceId}${ext}`;}
export async function sealComplianceEvidence(stagingPath:string,sealedPath:string,bytes:Buffer,contentType:string):Promise<void>{const container=getReportBlobServiceClient().getContainerClient("compliance-evidence");await container.getBlockBlobClient(sealedPath).upload(bytes,bytes.length,{blobHTTPHeaders:{blobContentType:contentType},conditions:{ifNoneMatch:"*"}});const sealed=await downloadComplianceEvidence(sealedPath);if(!sealed.equals(bytes))throw new Error("Sealed evidence does not match the verified bytes");await container.deleteBlob(stagingPath,{deleteSnapshots:"include"}).catch(()=>undefined);}
