...
export async function loadKpiTrust(pool: sql.ConnectionPool) {
  const table = await pool.request().query<{ ready: number; evidence_ready: number }>(`
    SELECT CASE WHEN OBJECT_ID('dbo.MissedTripFeedHealth', 'U') IS NULL THEN 0 ELSE 1 END AS ready,
           CASE WHEN COL_LENGTH('dbo.MissedTripFeedHealth', 'coverage_end_at') IS NULL THEN 0 ELSE 1 END AS evidence_ready
  `);
  if (table.recordset[0]?.ready !== 1) return resolveKpiTrust([]);
  const evidenceColumns = table.recordset[0]?.evidence_ready === 1
