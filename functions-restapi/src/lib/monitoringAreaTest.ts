...
export function formatMonitoringAreaTestMessage(input: { vehicleId: number; routeId: number | null; locationName: string; geofenceName: string; exitedAt: Date | string }): string {
...
export function monitoringAreaTestIsActive(expiresAt: Date | string, now = new Date()): boolean {
  return new Date(expiresAt).getTime() > now.getTime();
}

export function reportOccurredDuringMonitoringAreaTest(reportTimestamp: Date | string, activeSince: Date | string): boolean {
...
async function recordDeparture(pool: Awaited<ReturnType<typeof getPool>>, watch: Watch, position: Position): Promise<void> {
  const body = formatMonitoringAreaTestMessage({ vehicleId: position.vehicle_id, routeId: position.route, locationName: watch.location_name, geofenceName: watch.geofence_name, exitedAt: position.report_timestamp });
  ...
  await request.query(`
    ...
    FROM EventMonitoringAreaTestVehicleState WITH (UPDLOCK,HOLDLOCK) WHERE test_id=@test AND vehicle_id=@vehicle;
    IF @notify=1
    BEGIN
      UPDATE EventMonitoringAreaTestVehicleState SET last_notified_at=@exited WHERE test_id=@test AND vehicle_id=@vehicle;
      INSERT EventMonitoringAreaTestMessages(test_id,vehicle_id,route_id,exited_at,message_body) VALUES(@test,@vehicle,@route,@exited,@body);
    END;
    COMMIT TRANSACTION;
  `);
}
...
async function updateState(pool: Awaited<ReturnType<typeof getPool>>, watch: Watch, position: Position, inside: boolean, pending: boolean | null, confirmations: number): Promise<void> {
  ...
  await request.query(`UPDATE EventMonitoringAreaTestVehicleState SET is_inside=@inside,pending_is_inside=@pending,pending_confirmations=@confirmations,last_report_timestamp=@reported,updated_at=SYSUTCDATETIME() WHERE test_id=@test AND vehicle_id=@vehicle`);
}

export async function detectMonitoringAreaTests(context: InvocationContext): Promise<void> {
  ...
  const watches = (await pool.request().query<Watch>(`
    ...
    FROM EventMonitoringAreaTests t JOIN EventLocations l ON l.id=t.location_id JOIN EventGeofences g ON g.id=t.geofence_id
    WHERE t.is_enabled=1 AND t.expires_at>SYSUTCDATETIME() AND l.is_active=1 AND g.is_active=1
  `)).recordset;
  if (!watches.length) { await deliverPendingMonitoringAreaTestMessages(context); return; }
  ...
  for (const watch of watches) for (const position of positions) {
    ...
    const movements = prior && reportOccurredDuringMonitoringAreaTest(prior.report_timestamp, watch.active_since)
    ...
    if (!state) {
      ...
      await seed.query("INSERT EventMonitoringAreaTestVehicleState(test_id,vehicle_id,is_inside,last_report_timestamp) VALUES(@test,@vehicle,@inside,@reported)");
  ...
  await deliverPendingMonitoringAreaTestMessages(context);
}

export async function deliverPendingMonitoringAreaTestMessages(context: InvocationContext): Promise<void> {
  const pool = await getPool();
  const rows = (await pool.request().query<{ id: number; message_body: string; attempt_count: number }>(`
    ...
    FROM EventMonitoringAreaTestMessages m JOIN EventMonitoringAreaTests t ON t.id=m.test_id
    WHERE m.status='pending' AND (t.is_enabled=0 OR t.expires_at<=SYSUTCDATETIME() OR m.created_at<=DATEADD(HOUR,-24,SYSUTCDATETIME()));
    SELECT TOP (20) m.id,m.message_body,m.attempt_count FROM EventMonitoringAreaTestMessages m
    JOIN EventMonitoringAreaTests t ON t.id=m.test_id
  ...
  for (const row of rows) {
    ...
    await request.query("UPDATE EventMonitoringAreaTestMessages SET status=@status,sent_at=CASE WHEN @status='sent' THEN SYSUTCDATETIME() ELSE sent_at END,attempt_count=attempt_count+1,last_error=CASE WHEN @status='sent' THEN NULL ELSE @error END,next_attempt_at=CASE WHEN @status='pending' THEN DATEADD(SECOND,@delay,SYSUTCDATETIME()) ELSE NULL END WHERE id=@id AND status='pending'");
  }
}
