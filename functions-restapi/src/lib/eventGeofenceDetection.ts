...
export async function detectEventGeofenceCrossings(context: InvocationContext): Promise<void> {
  ...
  for (const position of positions) for (const fence of fences.filter((candidate) => candidate.service_plan_id === position.service_plan_id)) {
    ...
    if (!stable) continue;
    const transition = inside ? "enter" : "exit"; const ruleReq = pool.request(); ruleReq.input("plan", sql.UniqueIdentifier, fence.service_plan_id); ruleReq.input("fence", sql.UniqueIdentifier, fence.id); ruleReq.input("transition", sql.NVarChar, transition); const rules = (await ruleReq.query<DirectionRule>("SELECT rule.id,rule.geofence_id,rule.transition,rule.heading_min,rule.heading_max,rule.destination_label,rule.destination_location_id,rule.send_mode,rule.sort_order FROM (SELECT TOP (1) rules_json FROM EventServicePlanScopeSnapshots snapshot WHERE snapshot.service_plan_id=@plan ORDER BY snapshot.captured_at DESC) scope CROSS APPLY OPENJSON(scope.rules_json) WITH (id UNIQUEIDENTIFIER '$.id',geofence_id UNIQUEIDENTIFIER '$.geofence_id',transition NVARCHAR(10) '$.transition',heading_min FLOAT '$.heading_min',heading_max FLOAT '$.heading_max',destination_label NVARCHAR(200) '$.destination_label',destination_location_id UNIQUEIDENTIFIER '$.destination_location_id',send_mode NVARCHAR(10) '$.send_mode',sort_order INT '$.sort_order') rule WHERE rule.geofence_id=@fence AND rule.transition=@transition")).recordset; const rule = selectMatchingDirectionRule(rules, transition, position.heading);
