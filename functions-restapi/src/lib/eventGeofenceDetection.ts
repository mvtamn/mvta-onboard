...
async function insertCrossing(input: { pool: Awaited<ReturnType<typeof getPool>>; context: InvocationContext; position: Position; fence: Fence; transition: "enter" | "exit"; evidence: CrossingEvidence }): Promise<void> {
  ...
  const rules = (await ruleRequest.query<DirectionRule>(`
    SELECT scope_rule.id,scope_rule.geofence_id,scope_rule.name,scope_rule.transition,scope_rule.heading_min,scope_rule.heading_max,scope_rule.destination_label,scope_rule.destination_location_id,scope_rule.message_type,scope_rule.send_mode,scope_rule.sort_order
