...
import { readModel } from "./generateArtifact";
...
export async function issueFinal(pool: sql.ConnectionPool, input: { reportId: string; periodId: string; actor: string; recipient: string; deliveryMethod: string; senderAttestation: string; now?: Date; upload?: (path: string, html: string) => Promise<void> }): Promise<IssueOutcome> {
