import { observingSystem, observingSystemSql, parseOccurrenceSource, type ObservingSystem } from "../occurrenceIntake/sources";
...
// The occurrence intake module owns what a reference means.
export function outageSystemForSourceRef(sourceRef: string | null | undefined): ObservingSystem | null {
  return observingSystem(parseOccurrenceSource(sourceRef));
}
...
export function outageExclusionSql(alias: string): string {
  const o = alias;
  const system = observingSystemSql(`${o}.source_ref`);
