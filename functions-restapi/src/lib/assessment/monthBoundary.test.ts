...
test("a month nobody has opened is opened, computed, and drafted", () => {
  assert.deepEqual(monthBoundaryPlan({ now: new Date("2026-08-01T06:00:00Z"), priorStatus: null, currentStatus: null }), { openCurrent: "202608", openPrior: "202607", compute: "202607", draft: true });
});

test("a prior month still open, stale, or reopened is computed and drafted", () => {
  for (const status of ["open", "stale", "reopened"] as const) {
    assert.deepEqual(monthBoundaryPlan({ now: new Date("2026-08-01T06:00:00Z"), priorStatus: status, currentStatus: "open" }), { openCurrent: null, openPrior: null, compute: "202607", draft: true });
  }
});

test("a prior month in someone's hands is left alone", () => {
  for (const status of ["in_review", "in_validation", "finalized", "issued"] as const) {
    assert.deepEqual(monthBoundaryPlan({ now: new Date("2026-08-01T06:00:00Z"), priorStatus: status, currentStatus: "open" }), { openCurrent: null, openPrior: null, compute: null, draft: false });
  }
});
...
test("running twice on the same day does nothing the second time", () => {
  ...
  assert.deepEqual(monthBoundaryPlan({ now: new Date("2026-08-01T06:00:00Z"), priorStatus: "in_review", currentStatus: "open" }), { openCurrent: null, openPrior: null, compute: null, draft: false });
});
