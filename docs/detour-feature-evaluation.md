...
| Fulfillment modes and workflow lifecycle | **Implemented** | `avail`, `fixed_route_manual`, `mobility_manual`; `awaiting_fulfillment → fulfilled / fulfillment_failed → closed`, changed only through named acts in `lib/detourWorkflow/` (ADR-0030); readiness derived there. Console actions: record Avail entry, manual fallback, close. |
