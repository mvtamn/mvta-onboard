...
- **New app settings** `AVAIL_DETOURS_URL` (imperative, same convention as every other feed URL) - reuses the existing `AVAIL_AVL_REPORTS_API_KEY`; the owner has confirmed Detours does not need a separate production key (resolves the brief's own open question #6).
