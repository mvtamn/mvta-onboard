...
/** GET /me/access - the caller's Effective Access (ADR-0032). */
...
export interface MyAccess {
  person: { objectId: string | null; tenantId: string | null; name: string | null; email: string | null };
  roles: MyAccessRole[];
  /** Sorted `module.action` keys. The console gates on these and keeps no role lists. */
  actions: string[];
  /** One line per module, generated by the server from `actions`. */
  summary: string[];
  ingestion: boolean;
  rolesInOnBoard: boolean;
}

/** The module grid the Roles page draws (ADR-0032). */
export interface AccessCatalogModule {
  key: string;
  label: string;
  section: string;
...
export interface AccessRoleView {
  ...
  allActions: boolean;
  actions: string[];
  /** Generated by the server from the grid; never edited by hand. */
  summary: string[];
  members: number;
  seeded: boolean;
  archived: boolean;
...
export interface AccessRoleHistoryEntry {
  change: string;
...
export function createApiClient({ baseUrl, getToken, privilegedAuthenticationContext = "c1" }: ApiClientOptions) {
  ...
  return {
    ...
    /**
     * What the signed-in person may do. The console asks once and gates every
     * route, link and control on the answer, instead of reading roles from the
     * token and keeping its own lists of which roles a page needs - the copies
     * that drifted apart and left people on pages their data calls refused.
     */
    getMyAccess() {
    ...
    getAccessCatalog() {
      return request<{ modules: AccessCatalogModule[] }>("/api/manage/access/catalog", {}, true);
    },
    getAccessRoles() {
      return request<{ roles: AccessRoleView[] }>("/api/manage/access/roles", {}, true);
    },
    createAccessRole(input: { name: string; purpose: string; actions: string[] }) {
      return request<{ role: AccessRoleView }>("/api/manage/access/roles", { method: "POST", body: JSON.stringify(input) }, true);
    },
    updateAccessRole(key: string, input: { name?: string; purpose?: string; actions?: string[] }) {
      return request<{ role: AccessRoleView }>(`/api/manage/access/roles/${encodeURIComponent(key)}`, { method: "PATCH", body: JSON.stringify(input) }, true);
    },
    setAccessRoleArchived(key: string, archived: boolean) {
      return request<{ role: AccessRoleView }>(
        `/api/manage/access/roles/${encodeURIComponent(key)}/${archived ? "archive" : "restore"}`,
        { method: "POST" },
        true,
      );
    },
    getAccessRoleHistory(key: string) {
      return request<{ history: AccessRoleHistoryEntry[] }>(`/api/manage/access/roles/${encodeURIComponent(key)}/history`, {}, true);
    },
    ...
    getMonitoringAreaTests() { return request<{ tests: MonitoringAreaTest[]; teams_configured: boolean; teams_destination: string }>("/api/monitoring-area-tests", {}, true); },
    startMonitoringAreaTest(input: { location_id: string; geofence_id: string; duration_minutes: number }) { return request<{ tests: MonitoringAreaTest[]; teams_configured: boolean; teams_destination: string }>("/api/monitoring-area-tests", { method: "POST", body: JSON.stringify(input) }, true); },
    stopMonitoringAreaTest(id: string) { return request<void>(`/api/monitoring-area-tests/${id}`, { method: "DELETE" }, true); },
    ...
    // The archived bytes of one artifact, hash-verified by the server. Not
    // JSON, so neither goes through request(). Preview reads them as text;
    // Download keeps the Blob untouched so the saved file still hashes to
    // content_sha256.
    async getAssessmentReportHtml(id: string) {
      return (await requestBlob(`/api/assessment-reports/${id}/html`)).text();
    },
    getAssessmentReportDownload(id: string) {
      return requestBlob(`/api/assessment-reports/${id}/download`);
    },
