...
import { deliveryClass, deliveryLabel, receiptLabel } from "../components/DetourDeliveryRecord.js";
...
describe("deliveryLabel", () => {
  it("names server delivery states, with Posted for Teams", () => {
    ...
    expect(deliveryLabel(comm({ delivery_status: "sent", delivery_completed_at: null }))).toBe("Accepted by provider");
    expect(deliveryLabel(comm({ delivery_status: "delivered", delivery_completed_at: null }))).toBe("Delivered");
...
describe("receiptLabel", () => {
  it("names each provider receipt state", () => {
    expect(receiptLabel("accepted")).toBe("Accepted, awaiting receipt");
    expect(receiptLabel("delivered")).toBe("Delivered");
    expect(receiptLabel("filtered_spam")).toBe("Filtered as spam");
    expect(receiptLabel("bounced")).toBe("Bounced");
  });
});
