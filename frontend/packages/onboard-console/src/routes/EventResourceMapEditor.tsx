...
export function EventResourceMapEditor() {
  const [geofences, setGeofences] = useState<StoredFence[]>([]); const [locations, setLocations] = useState<EventLocation[]>([]); const [plans, setPlans] = useState<EventServicePlan[]>([]); const [selectedFence, setSelectedFence] = useState(""); const [selectedLocation, setSelectedLocation] = useState(""); const [rule, setRule] = useState<Partial<EventGeofenceRule>>({ transition: "exit", heading_min: 0, heading_max: 360, send_mode: "manual", destination_label: "" }); const [planId, setPlanId] = useState(""); const [planName, setPlanName] = useState(""); const [message, setMessage] = useState<string | null>(null);
  const load = () => Promise.all([api.getEventGeofences(), api.getEventLocations(), api.getEventServicePlans()]).then(([g, l, p]) => { setGeofences(g.geofences); setLocations(l.locations); setPlans(p.plans); if (!planId && p.plans[0]) setPlanId(p.plans[0].id); }).catch((err) => setMessage(err instanceof ApiError ? err.message : "Event resources are unavailable until migrations 033 and 034 are applied."));
  useEffect(() => { void load(); }, []);
