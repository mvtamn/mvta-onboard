      ...
      <tbody>{geofences.map((geofence) => {
        ...
        return <tr key={geofence.id}>
          ...
          <td>{geofence.is_active
            ? <button className="btn-sm danger" disabled={busy === geofence.id} aria-label={`Deactivate Monitoring Area ${geofence.name}`} onClick={() => void remove(geofence)}>{busy === geofence.id ? "Deactivating…" : "Deactivate"}</button>
            : "—"}</td>
        </tr>;
      })}</tbody>
    </table>
