...
export function EventResourceMapEditor() {
  ...
  async function saveRule() {
    ...
    try {
      ...
      setMessage(editingRuleId ? "Direction rule updated." : "Direction rule saved."); setEditingRuleId(null); setRule({ transition: "exit", heading_min: 0, heading_max: 360, send_mode: "manual", destination_label: "", sort_order: 0 }); setDirectionPreset("any"); await load();
  ...
  const previewMessage = selected && rule.destination_label?.trim()
    ? `Bus ${testBus || "1234"} ${rule.transition === "enter" ? "entered" : "exited"} ${selected.name}; ${rule.destination_label.trim()}.`
  ...
  return <>
    ...
    <div className="panel-body event-direction-rules">
      <p className="panel-desc">Every crossing in an active operating scope creates an Event AVL message. This rule controls the message details and whether the message stays for review or is also sent automatically to the configured Teams channel. Lower priority wins when rules overlap.</p>
      <div className="event-rule-step"><span className="event-rule-step-number">1</span><div><strong>When should this rule match?</strong><div className="event-rule-fields"><label>Geofence<select className="f" value={selectedFence} onChange={(e) => setSelectedFence(e.target.value)}><option value="">Select a geofence</option>{geofences.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select></label><label>Boundary movement<select className="f" value={rule.transition} onChange={(e) => setRule((r) => ({ ...r, transition: e.target.value as "enter" | "exit" }))}><option value="enter">Vehicle enters</option><option value="exit">Vehicle exits</option></select></label><label>Travel direction<select className="f" value={directionPreset} onChange={(e) => chooseDirection(e.target.value as CompassDirection)} aria-label="Vehicle travel direction"><option value="any">Any direction</option>{Object.keys(COMPASS_RANGES).map((direction) => <option key={direction} value={direction}>{direction}</option>)}<option value="custom">Custom compass range</option></select></label></div></div></div>
      ...
      <div className="event-rule-step"><span className="event-rule-step-number">3</span><div><strong>What should Event AVL say or do?</strong><div className="event-rule-fields"><label>Operational message<input className="f" maxLength={200} value={rule.destination_label ?? ""} onChange={(e) => setRule((r) => ({ ...r, destination_label: e.target.value }))} placeholder="Example: Proceed to Gate A" /></label><label>Related location<select className="f" value={rule.destination_location_id ?? ""} onChange={(e) => setRule((r) => ({ ...r, destination_location_id: e.target.value || null }))}><option value="">No location linked</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>Delivery behavior<select className="f" value={rule.send_mode} onChange={(e) => setRule((r) => ({ ...r, send_mode: e.target.value as "manual" | "auto" }))}><option value="manual">Create review item in Event AVL</option><option value="auto">Send automatically and show in Event AVL</option></select></label></div></div></div>
      ...
      <div className="actions"><button className="btn-primary" disabled={!selectedFence || !rule.destination_label?.trim() || saving} onClick={() => void saveRule()}>{saving ? "Saving…" : editingRuleId ? "Save rule changes" : "Save direction rule"}</button>{editingRuleId && <button className="btn-sm" onClick={() => { setEditingRuleId(null); setRule({ transition: "exit", heading_min: 0, heading_max: 360, send_mode: "manual", destination_label: "", sort_order: 0 }); setDirectionPreset("any"); }}>Cancel edit</button>}</div>
      {selected && <div style={{ marginTop: 12 }}><strong>{selected.name} rules</strong>{(selected.rules ?? []).length === 0 ? <p className="muted">No rules configured.</p> : <table className="data"><thead><tr><th>Priority</th><th>Movement</th><th>Direction</th><th>Message</th><th>Delivery</th><th>Action</th></tr></thead><tbody>{(selected.rules ?? []).map((item) => <tr key={item.id}><td>{item.sort_order}</td><td>{item.transition === "enter" ? "Enters" : "Exits"}</td><td>{item.heading_min}°–{item.heading_max}°</td><td>{item.destination_label}</td><td>{item.send_mode === "auto" ? "Automatic" : "Event AVL review"}</td><td><button className="btn-sm" onClick={() => editRule(item)}>Edit</button>{" "}<button className="btn-sm" onClick={() => void deleteRule(item.id)}>Delete</button></td></tr>)}</tbody></table>}</div>}
    </div>
  </>;
}
