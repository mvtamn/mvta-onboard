...
export function EventResourceMapEditor() {
  const [geofences, setGeofences] = useState<StoredFence[]>([]); const [locations, setLocations] = useState<EventLocation[]>([]); const [plans, setPlans] = useState<EventServicePlan[]>([]); const [routes, setRoutes] = useState<{ route_id: number; label: string | null; category: string }[]>([]); const [selectedFence, setSelectedFence] = useState(""); const [selectedLocation, setSelectedLocation] = useState(""); const [selectedRoute, setSelectedRoute] = useState(""); const [rule, setRule] = useState<Partial<EventGeofenceRule>>({ transition: "exit", heading_min: 0, heading_max: 360, send_mode: "manual", destination_label: "" }); const [directionPreset, setDirectionPreset] = useState<CompassDirection>("any"); const [planId, setPlanId] = useState(""); const [planName, setPlanName] = useState(""); const [message, setMessage] = useState<string | null>(null);
  ...
  async function link(kind: "geofences" | "locations") { const value = kind === "geofences" ? selectedFence : selectedLocation; if (!planId || !value) return; try { await api.linkEventServicePlan(planId, kind, value); setMessage(`${kind === "geofences" ? "Geofence" : "Location"} linked to service plan.`); void load(); } catch (err) { setMessage(err instanceof ApiError ? err.message : "Could not link service plan resource."); } }
