...
export function EventResourceMapEditor() {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => {
    if (!selectedFence || selectedFence !== deepLinkedFence.current) return;
  ...
  function resetRuleEditor() { setEditingRuleId(null); setRule({ name: "", transition: "exit", heading_min: 0, heading_max: 360, message_type: "custom", send_mode: "manual", destination_label: "", sort_order: 0 }); setDirectionPreset("any"); }
  async function saveRule() {
    ...
    try {
      ...
      setMessage(editingRuleId ? "Direction rule updated." : "Direction rule saved."); resetRuleEditor(); await load();
