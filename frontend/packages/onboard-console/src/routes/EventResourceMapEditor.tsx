...
function MapEditor({ geofences, locations, plans, onChanged }: { geofences: StoredFence[]; locations: EventLocation[]; plans: EventServicePlan[]; onChanged: () => void }) {
  ...
  async function saveDraft() {
    ...
    try {
      if (draft.kind === "geofence") { const geometry = geometryOf(draft.shape); if (!geometry) return; const validationError = validateDrawnPolygon(geometry); if (validationError) { setNotice(`Geofence not saved: ${validationError}. Draw a new boundary.`); return; } await api.createEventGeofence({ name: name.trim(), polygon: geometry, purpose }); }
