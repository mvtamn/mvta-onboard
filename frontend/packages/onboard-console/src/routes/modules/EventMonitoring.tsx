...
export function EventMonitoring({ fieldView = false }: { fieldView?: boolean }) {
  ...
  if (fieldView) return <EventAVLFieldView vehicles={allVehicles} notifications={eventQueue} selectedVehicleId={selectedVehicleId} onSelectVehicle={setSelectedVehicleId} mapGeofences={visibleGeofences} mapLocations={visibleLocations} mapStyle={mapStyle} traffic={traffic} staleCount={allVehicles.filter((vehicle) => vehicle.is_stale).length} trustState={state} actionsBlocked={actionsBlocked} canManageNotificationActions={canManageNotificationActions} onReviewNotification={reviewNotification} refreshing={refreshing} onRefresh={() => void refresh()} />;
...
function EventAVLFieldView({ vehicles, notifications, selectedVehicleId, onSelectVehicle, mapGeofences, mapLocations, mapStyle, traffic, staleCount, trustState, actionsBlocked, canManageNotificationActions, onReviewNotification, refreshing, onRefresh }: { vehicles: EventVehiclePosition[]; notifications: EventGeofenceNotification[]; selectedVehicleId: number | null; onSelectVehicle: (id: number) => void; mapGeofences: EventGeofence[]; mapLocations: EventLocation[]; mapStyle: MapStyle; traffic: boolean; staleCount: number; trustState: ReturnType<typeof deriveEventMonitoringDataState>; actionsBlocked: boolean; canManageNotificationActions: boolean; onReviewNotification: (id: string, action: "acknowledge" | "send" | "dismiss") => Promise<void>; refreshing: boolean; onRefresh: () => void }) {
