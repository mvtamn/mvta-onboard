...
export function EventMonitoring({ fieldView = false }: { fieldView?: boolean }) {
  ...
  const fieldViewQuery = new URLSearchParams();
  if (selectedEventId) fieldViewQuery.set("event", selectedEventId);
  if (selectedPlanId) fieldViewQuery.set("plan", selectedPlanId);
  const fieldViewUrl = `/console/events/avl/field${fieldViewQuery.size ? `?${fieldViewQuery}` : ""}`;
...
function EventAVLFieldView({ vehicles, notifications, selectedVehicleId, onSelectVehicle, mapGeofences, mapLocations, showGeofences, onShowGeofencesChange, showLocations, onShowLocationsChange, mapStyle, onMapStyleChange, traffic, onTrafficChange, fieldViewUrl, staleCount, trustState, actionsBlocked, canManageNotificationActions, onReviewNotification, refreshing, onRefresh }: {
  ...
  return <section className="evmon evmon-field" aria-label="Event AVL field view">
    <div className="evmon-field-header"><strong>Event AVL field view</strong><span>{trustState.tone === "success" ? "Feed trusted" : trustState.title} · {staleCount} stale</span><RefreshLiveDataButton refreshing={refreshing} onRefresh={onRefresh} /><button className="evmon-notification-badge" type="button" aria-label={`${notifications.length} items in the Event AVL status queue`} onClick={() => setShowNotifications(true)}>Status queue <strong>{notifications.length}</strong></button></div>
