...
describe("Caps", () => {
  ...
  it("withdraws a required determination with a reason on the audit row", async () => {
    (api.getAssessmentCaps as ReturnType<typeof vi.fn>).mockResolvedValue({ caps: [cap()] });
    (api.transitionAssessmentCap as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "cap1", status: "withdrawn" });
    render(<Caps period={period} rows={[]} />);
    await screen.findByText("Fixed-route OTP");
    await userEvent.click(screen.getByRole("button", { name: "Withdraw" }));
    await waitFor(() => expect(api.transitionAssessmentCap).toHaveBeenCalledWith("cap1", "withdrawn", { note: "Verified two clean months" }));
  });

  it("requires a plan by hand on an issued month, MVTA's initiative unless the contractor brought it", async () => {
    (api.getAssessmentCaps as ReturnType<typeof vi.fn>).mockResolvedValue({ caps: [] });
    (api.createAssessmentCap as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "new", due_at: "2026-09-17T00:00:00Z" });
    render(<Caps period={period} rows={[]} />);
    await userEvent.click(await screen.findByRole("button", { name: "Require a plan" }));
    await waitFor(() => expect(api.createAssessmentCap).toHaveBeenCalledWith({ period_id: "p1", trigger_reason: "discretionary", note: "Verified two clean months" }));
  });
});
