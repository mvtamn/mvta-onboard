...
export function Relief({ period, onChanged }: { period: AssessmentPeriod | undefined; onChanged: () => void }) {
  const { can } = useAccess(); const { prompt } = useAppDialog();
  ...
  // Late notice disqualifies relief; approving anyway is a recorded exception with its own reason.
  const decide = async (c: ExcusableDelayClaim, status: "approved" | "denied") => { const note = await prompt({ title: status === "approved" ? "Approve claim" : "Deny claim", label: "Decision note", confirmLabel: status === "approved" ? "Approve" : "Deny", multiline: true, required: true }); if (!note) return; let override: string | undefined; if (status === "approved" && c.late_notice) { const why = await prompt({ title: "Notice was late", description: "Notice arrived more than 24 hours after the event, which disqualifies relief. Approving anyway is an exception that needs its own reason.", label: "Why relief is granted despite late notice", confirmLabel: "Approve as an exception", multiline: true, required: true }); if (!why) return; override = why; } void run(() => api.decideExcusableDelayClaim(c.id, status, note, override)); };
  ...
  return <section className="assessment-relief">
    ...
    {writer && <div className="assessment-cap-form">
    ...
    {outages.length ? outages.map(o => <div className="assessment-card" key={o.id}>
      ...
      {!o.ended_at && writer && <button onClick={() => void run(() => api.endSystemOutage(o.id, new Date().toISOString()))}>End window</button>}
