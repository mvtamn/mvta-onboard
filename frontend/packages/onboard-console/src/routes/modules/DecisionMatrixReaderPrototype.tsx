...
function DocumentSupport({ procedure, previewState, setPreviewState }: { procedure: Procedure; previewState: PreviewState; setPreviewState: (state: PreviewState) => void }) {
  return <section className="dmxp-documents" aria-label="Document and visual support"><h3>Document support</h3><div className={`dmxp-health ${procedure.health.toLowerCase().replaceAll(" ", "-")}`}><strong>{procedure.health}</strong><p>{procedure.healthDetail}</p></div>{procedure.hasRendition ? <VisualPreview state={previewState} setState={setPreviewState} /> : <p className="dmxp-preview-fallback">No approved PNG/JPEG rendition is available. The structured Procedure remains the operating guidance.</p>}<QrgInlineViewer />{procedure.sourceAvailable ? <div className="dmxp-source-actions"><a className="dmxp-source" href="https://mvtamn.sharepoint.com/sites/Operations/" target="_blank" rel="noreferrer">Open SOP in SharePoint <span aria-hidden="true">↗</span></a><a className="dmxp-source" href="https://mvtamn.sharepoint.com/sites/Operations/" target="_blank" rel="noreferrer">Open REF in SharePoint <span aria-hidden="true">↗</span></a><a className="dmxp-source" href="https://mvtamn.sharepoint.com/sites/Operations/" target="_blank" rel="noreferrer">Open APP in SharePoint <span aria-hidden="true">↗</span></a></div> : <p className="dmxp-source-unavailable">SOP, REF, and APP documents are currently unavailable in SharePoint</p>}<p className="dmxp-supporting-note">The QRG is supporting guidance only; it never replaces the governing SOP or REF.</p></section>;
}

function QrgInlineViewer() {
  const [open, setOpen] = useState(false);
  return <section className="dmxp-qrg-inline"><button type="button" aria-expanded={open} onClick={() => setOpen((current) => !current)}>{open ? "Close QRG inline view" : "Open QRG inline view"}</button>{open ? <div className="dmxp-qrg-document" aria-label="QRG inline document view"><header><div><span>QRG · SHAREPOINT DOCUMENT</span><h4>Controller quick reference</h4></div><a href="https://mvtamn.sharepoint.com/sites/Operations/" target="_blank" rel="noreferrer">Open QRG in SharePoint <span aria-hidden="true">↗</span></a></header><p className="dmxp-qrg-placeholder">Prototype inline viewer — production will stream the controller-authorized SharePoint QRG without exposing document credentials or replacing the Procedure text.</p><ol><li>Protect the immediate safety condition.</li><li>Complete the first required action in the Procedure.</li><li>Use the governing SOP or REF for the complete approved source.</li></ol></div> : null}</section>;
}
