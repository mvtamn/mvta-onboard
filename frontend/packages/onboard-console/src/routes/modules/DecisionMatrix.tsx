...
// An empty Matrix has four causes and a controller under pressure needs to
// know which one they are looking at: the request failed, the database has no
// Procedure tables, nothing has been approved yet, or this search matched
// nothing. Only the last is business as usual.
type LoadState = "loading" | "unavailable" | "not_connected" | "unpublished" | "no_match" | "live";
function loadState(procedures: DecisionMatrixReaderProcedure[] | null, diagnostics: DecisionMatrixDiagnostics | null, failed: boolean, searching: boolean): LoadState {
  if (failed) return "unavailable";
  if (!procedures || !diagnostics) return "loading";
  if (!diagnostics.table_ready) return "not_connected";
  if (procedures.length) return "live";
  return searching ? "no_match" : "unpublished";
}

// decisionMatrix.css carries a severity rail on every row and card - red for
// Stop service, orange for Restrict service, green for routine - and nothing
// was setting it, so every Procedure read as neutral. A severity the database
// does not constrain to these three keeps the neutral rail rather than
// borrowing a colour it has not earned.
const SEVERITY_CLASS: Record<string, string> = { "Stop service": "stop", "Restrict service": "restricting", "Routine / no escalation": "clear" };
function severityClass(severity: string): string { return SEVERITY_CLASS[severity] ?? ""; }
...
export function DecisionMatrix() {
  ...
  const [diagnostics, setDiagnostics] = useState<DecisionMatrixDiagnostics | null>(null);
  ...
  useEffect(() => {
    api.getDecisionMatrix({ q: query || undefined }).then((result) => { setProcedures(result.procedures); setDiagnostics(result.diagnostics); setError(null); }).catch((reason) => { setProcedures([]); setDiagnostics(null); setError(reason instanceof ApiError ? reason.message : "Decision Matrix content is temporarily unavailable."); });
  }, [query]);

  const state = loadState(procedures, diagnostics, error !== null, query.length > 0);
...
function ProcedureReader({ procedure, actionFirst, expanded, onSelect }: { procedure: DecisionMatrixReaderProcedure; actionFirst: boolean; expanded: boolean; onSelect: () => void }) {
  ...
  return <article className={`row ${severityClass(procedure.severity)}`.trim()}><div className="rowbody"><div className="card-top"><h2>{procedure.condition}</h2><span className="sr-only">Severity: {procedure.severity}</span><button type="button" className="btn-sm" onClick={onSelect} aria-pressed={expanded}>{expanded ? "Selected" : "Read Procedure"}</button></div>{actionFirst ? <><section className="field"><div className="flabel">Immediate actions</div><ol className="action-list">{procedure.immediate_actions.map((action) => <li key={action.id}>{action.instruction}</li>)}</ol></section><section className="field"><div className="flabel">Criteria</div><ul className="fvalue">{procedure.criteria.map((criterion) => <li key={criterion.id}>{criterion.kind === "excludes" ? "Does not apply: " : "Applies: "}{criterion.text}</li>)}</ul></section></> : content}{expanded ? <ProcedureDetails procedure={procedure} /> : null}</div></article>;
}
...
function ProcedureCard({ procedure, onSelect }: { procedure: DecisionMatrixReaderProcedure; onSelect: () => void }) { return <article className={`card ${severityClass(procedure.severity)}`.trim()}><div className="card-top"><h2>{procedure.condition}</h2><span className="sr-only">Severity: {procedure.severity}</span><button className="btn-sm" type="button" onClick={onSelect}>Read</button></div><div className="cfield"><span className="clabel">First action</span>{procedure.immediate_actions[0]?.instruction ?? "No action published"}</div><div className="cfield"><span className="clabel">Criteria</span>{procedure.criteria[0]?.text ?? "No criteria published"}</div><div className="ctags">{procedure.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></article>; }
