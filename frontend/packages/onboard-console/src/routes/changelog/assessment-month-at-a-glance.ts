  ...
  sections: [
    {
      ...
      items: [
        ...
        "The scorecard reads in the contract's order: grouped by category, the categories as Lists orders them, with a totals row. Monthly metrics is a checklist rather than a form above a history table: one line per hand-entered figure the month scores, missing ones first, each entered in place with its source of record and stamped with who entered it and when; the standards the month does not score are listed beneath with a way to change the assignments.",
        "Under Administration, the four Performance Assessment pages no longer repeat their title beneath the page header. Lists uses the same master-detail workspace as the other three, its twelve lists down the left with system lists marked. Contractors shows the Agreements under the selected contractor. Agreements groups its form into Term, Contract references and Process, with the units inside the fields. Standards groups its catalog by category and offers the catalog-or-Agreement ladder choice as a segmented control.",
        ...
        "The card uses the glossary's own words: the lifecycle reads Open, Under Review, In Validation, Finalized, Issued, and a stale month stands at Under Review where its pill already says the inputs moved.",
        "At In Validation with items still pending the card offered \u201cContinue review\u201d, and every control on that page is gated on In Review \u2014 the reader arrived somewhere nothing could be pressed. It now says what is in the way and shows it, and the reviewed tally counts only rows a reviewer can act on, so it can reach its total.",
      ],
    },
  ],
};
