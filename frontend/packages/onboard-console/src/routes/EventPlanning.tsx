...
export function EventPlanning() {
  ...
  const [geofences, setGeofences] = useState<ResourceOption[]>([]);
    ...
    {plan && <>
      ...
      <div className="panel-body">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <select className="f" value={geofenceId} onChange={(e) => setGeofenceId(e.target.value)}><option value="">Select geofence</option>{geofences.map((row) => <option key={row.id} value={row.id}>{row.label}</option>)}</select><button className="btn-sm" disabled={!geofenceId || (!editable && !revision)} onClick={() => void link("geofences")}>Add geofence</button>
