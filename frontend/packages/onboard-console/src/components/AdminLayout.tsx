...
import { IconClock, IconGear, IconShield, IconUsers, IconBus, IconWrench } from "./NavIcons.js";

const ADMIN = ["OCC.Admin"] as const;
const ACCESS = ["OCC.Admin", "OCC.AccessAdmin"] as const;

const links = [
  { to: "/admin", label: "Overview", icon: IconGear, end: true, roles: ACCESS },
      ...
      <aside className="admin-secondary-nav" aria-label="Administration">
        ...
        <nav className="admin-secondary-links">
          {visibleLinks.map((link) => {
            ...
            return <NavLink key={to} to={to} end={"end" in link ? link.end : false}>
              <Icon />
              <span>{label}</span>
            </NavLink>;
          })}
